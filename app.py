"""
NSE MARKET INTELLIGENCE — REAL-TIME STREAMLIT DASHBOARD
=======================================================
...
LAYOUT OF THIS FILE
    ...
    2. Page config
    3. Global CSS
    4. Cached loaders      — prices, news, sentiment, models, macro
    5. Prediction logic    — crash probability, model labelling
    6. Chart builders      — price, sentiment, gauge
    7. Loading screens     — full-page splash + inline placeholders
    8. Page body           — ticker tape, controls, status, metrics, tabs
"""
...
EXPORT_DIR = DATA_DIR / "exports"     # csv artifacts, incl. macro_daily.csv
MODEL_DIR = DATA_DIR / "models"       # rf / xgb / label encoder / stats json
LEGACY_MODEL_DIR = ROOT_DIR / "models"  # stacking ensemble + its meta scaler

# Make `src` importable no matter which directory streamlit was launched from.
# ROOT_DIR  -> `from src.xxx import ...`
# SRC_DIR   -> `import training.macro` for the on-demand macro rebuild
...
def load_ensemble():
    ...
    candidates = [
        LEGACY_MODEL_DIR / "stacking_ensemble.pkl",
...
def load_scaler_and_features():
    ...
    feature_cols = None
    feature_path = LEGACY_MODEL_DIR / "feature_cols.pkl"
...
def load_label_encoder():
    ...
    candidates = [
        LEGACY_MODEL_DIR / "label_encoder.pkl",
...
def ensure_macro_fresh(max_age_hours: int = 24) -> Path | None:
    ...
    Rebuilds live via src/training/macro.py, which writes to data/exports/.
    """
    candidate_paths = [EXPORT_DIR / "macro_daily.csv"]
    ...
    try:
        macro_mod = importlib.import_module("training.macro")
    ...
    if built_path.exists():
        if built_path != candidate_paths[0]:
            try:
                shutil.copy(built_path, candidate_paths[0])
            except Exception as exc:
                print(f"[MACRO] Copy to data/exports failed: {exc}")
        return built_path
    return None
...
def load_macro_latest(target_date=None) -> dict:
    ensure_macro_fresh()
    paths = [EXPORT_DIR / "macro_daily.csv"]
...
# ═════════════════════════════════════════════════════════════════════════════
# 5. PREDICTION LOGIC
#    Crash probability (ensemble, or heuristic when no model is available)
# ═════════════════════════════════════════════════════════════════════════════
...
# Shared styles + markup for the small inline "working..." placeholders that
# appear while price data and sentiment are being fetched.
MINI_LOADER_CSS = """
<style>
...
def render_mini_loader(placeholder, title: str, status: str) -> None:
    """Draw the small animated placeholder into a st.empty() container."""
    placeholder.markdown(
        MINI_LOADER_CSS + MINI_LOADER_BODY.format(title=title, status=status),
        unsafe_allow_html=True,
    )
