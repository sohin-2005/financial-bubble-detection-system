...
def load_data():
    ...
    for df in [train_df, val_df, test_df]:
        if "polarity_score" in df.columns:
            df.rename(columns={"polarity_score": "daily_sentiment_index"}, inplace=True)
    
    # Fill any remaining missing sentiment with neutral
    train_df["daily_sentiment_index"] = train_df["daily_sentiment_index"].fillna(0.0)
    val_df["daily_sentiment_index"]   = val_df["daily_sentiment_index"].fillna(0.0)
    test_df["daily_sentiment_index"]  = test_df["daily_sentiment_index"].fillna(0.0)
    ...
    print(f"  Train rows: {len(train_df)} ({before - after} dropped for missing values)")
...
def walk_forward_validation(df, features):
    ...
    for train_end, test_start, test_end in windows:
        ...
        if len(np.unique(train["label"].values)) < 3:
            print(f"  Skipping {test_start}→{test_end} — missing classes in train")
            continue
